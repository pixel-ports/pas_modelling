            ...
            while len(ids_to_process) > 0:
                for id in ids_to_process:
                    ...
                    if operation["CALCULATION"]["Start_Type"] == "ASAP":
                        ...
                        assert (
                            "Case_0" in operation["CALCULATION"]["Start_Type"].keys()
                        ...
                        conditions_ids = [
                            list(condition.values())[0] for condition in conditions
                        ]
                    ...
                    if operation_processed:
                        if (
                            list(operation["CALCULATION"]["Duration_Type"].keys())[0]
                            == "Fixe"
                        ):
                            assert (
                                operation["CALCULATION"]["Duration_Type"]["Fixe"][
                                    "Unit"
                                ]
                                == "min"
                            ...
                            assert (
                                operation["CALCULATION"]["Duration_Type"]["Amount"][
                                    "Unit"
                                ]
                                == "%"
                            )
                            assert (
                                "Value" in machine["SPECIFICATION"]["Throughput"].keys()
                            ), (
