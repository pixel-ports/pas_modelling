...
def main(PAS_instance=None, LOGS=None, HANDLINGS=None, local_export=False):
	...
	output_content = {
		"handlings":HANDLINGS,
		#"logs":LOGS #FIXME problème d'export ds ES de l'array qui contient des objets (mapping non supporter). Besoin de drop pr le moment, avant de restructuré le logs
...
def put_IH(output_content, output_call): #TODO ajouter retour requet TODO vérifier format TS
	...
	time_label = str(now.date()) +"_"+ str(now.hour) +"h"+ str(now.hour) +"m"+ str(now.hour)
	...
	if doc_id =='':
		doc_id = "output_" + time_label
	document = output_content
	#Elasticsearch().indices.delete(index= index_id)
	# document = {
        # "info":{
        #     "ID": doc_id,
        #     'nature': "output",
