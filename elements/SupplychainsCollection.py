...
class SupplychainsCollection:
    ...
    def get_default(self) -> Dict:
        default_supplychains = [sc for sc in self.list if sc["identification"]["name"] == "defaultSupplychain"]
    ...
    def map_supplychain(self, cargo) -> (Dict, str):
        filtered_supplychains: List[Dict] = [sc for sc in self.list if (cargo["type"] in sc["identification"]["suitableCargoType"])]
        ...
        if len(filtered_supplychains) == 1:
            ...
            max_priority_chains: List[Dict] = [sc for sc in filtered_supplychains if sc["identification"]["priority"] == max_priority]
    ...
    def get_operation(self, supplychain, operation_id: int) -> Dict:
        filtered_operations = [op for op in supplychain["operationsSequence"] if op["id"] == operation_id]
    ...
    def adjust_starting_ts(self, supplychain: Dict, operation: Dict, starting_ts: int):
        if operation["startOption"]["type"] == "fixedDelay":
            ...
            target_operation = self.get_operation(supplychain, operation["startOption"]["value"])
