...
class MachinesCollection:
    ...
    def get(self, id: int) -> Dict:
        return next((machine for machine in self.list if machine["identification"]["id"] == id), None)
    ...
    def get_throughput(self, machine, cargo_type: str, distance: float) -> float:
        ...
        if distance ==  0:
            ...
            speed_machine_empty: float = self.get_specifications(machine, "empty")["throughput"]["moving"]["speed"]
    ...
    def get_overlapping_TS(self, machine, startTS: int, endTS: int) -> (int, int):  # TODO Add machine working time constraints
    ...
    def is_available(self, machine, startTS: int, endTS: int) -> bool:
        return self.get_overlapping_TS(machine, startTS, endTS) == (None, None)  # TODO check return type
    ...
    def add_use(self, machine, startTS: int, endTS: int, duration: int, operation: Dict, supplychain: Dict, cargo: Dict) -> None:
    ...
    def get_timeserie(self, machine) -> Dict:
        timeserie: Dict = {
            "machineId": machine["identification"]["id"],
            "uses": []
        }
