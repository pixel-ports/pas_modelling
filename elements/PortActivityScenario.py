...
class PortActivityScenario():
    ...
    def __init__(self, ships_call_list: ShipsCallList, machines_collection: MachinesCollection, supplychains_collection: SupplychainsCollection):
    ...
    def execute_step(self, step: int) -> None:
        ...
        if step == 2:
            ...
            self.export_to_json(self.activities_queue.list, os.getenv("ACTIVITIES_QUEUE"))
        elif step > 2:
    ...
    def step_1(self) -> CargoesQueue:
        ...
        return CargoesQueue(sorted(cargoes, key=lambda x: -x["ship"]["constraints"]["priority"]))
    ...
    def step_2(self) -> ActivitiesQueue:
        ...
        for cargo in self.cargoes_queue.list:
            selected_supplychain, mapping_type = self.supplychains_collection.map_supplychain(cargo)
    ...
    def step_3(self) -> "PortActivityScenario":
        ...
        for activity in self.activities_queue.list:
            ...
            for operation in supplychain["operationsSequence"]:
                ...
                operation["throughput"]: float = self.machines_collection.get_throughput(machine, cargo["type"], operation["distance"])
                duration_use: int = cargo["amount"]/operation["throughput"]

                startingTS = self.supplychains_collection.adjust_starting_ts(supplychain, operation, startingTS)
                endingTS: int = startingTS + duration_use  # TODO Implement other operations specific durations
                startingTS, endingTS = self.machines_collection.get_next_available_TS(machine, startingTS, endingTS)
                self.machines_collection.add_unavailable_period(machine, startingTS, endingTS)
        ...
        for _, machines in machines.items():
            pas_dict["timeseries"].append(self.machines_collection.get_timeserie(machine))
        return pas_dict
